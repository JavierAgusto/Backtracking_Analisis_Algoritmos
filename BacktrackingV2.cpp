/*Primer trabajo Backtracking v2 */
...
// Comprobar si la casilla está en el camino mínimo.
bool isInPath(int x, int y, const vector<pair<int, int>>& path) {
...
void findMinCost(int x, int y, int cost, vector<pair<int, int>>& path) {
    ...
    if (x == N - 1 && y == N - 1) {// Comprobar si hemos llegado a la casilla de destino (esquina inferior derecha).
        if (cost < minCost) {// Comprobar si el costo actual es menor que el costo mínimo encontrado hasta ahora.
            minCost = cost;// Actualizar el costo mínimo encontrado hasta ahora.
            minPath = path;// Actualizar el camino mínimo encontrado hasta ahora.
        }
    ...
    int dx[] = {1, 0};// Movimientos en el eje x (derecha y abajo).
    int dy[] = {0, 1};// Movimientos en el eje y (derecha y abajo).
    // Recorrer los movimientos posibles y llamar recursivamente a la función
    for (int i = 0; i < 2; i++) {
        int newX = x + dx[i];// Calcular la nueva coordenada x de la casilla.
        int newY = y + dy[i];// Calcular la nueva coordenada y de la casilla.
        // Comprobar si la casilla es válida y si el costo actual es menor que el costo mínimo encontrado hasta ahora.
        if (isValid(newX, newY)) {
            int newCost = cost + abs(matriz[newX][newY] - matriz[x][y]);// Calcular el nuevo costo.
            if (newCost >= minCost) {
                continue;
            }
...
// Encontrar el camino mínimo sin poda en las cuatro direcciones o vecindades .
void FindMinCostFour(int x,int y ,int cost,vector<pair<int,int >>& path){
    //cout << "Explorando (" << x << ", " << y << "), Costo: " << cost << endl; // Depuración.
    if (x==N-1 && y==N-1){
        if (cost<minCost){// Comprobar si el costo actual es menor que el costo mínimo encontrado hasta ahora.
            minCost=cost;// Actualizar el costo mínimo encontrado hasta ahora.
    ...
    // Movimientos posibles: arriba, derecha, abajo e izquierda.
    int dx[]={-1,0,1,0};// Movimientos en el eje x (arriba, derecha, abajo e izquierda).
    int dy[]={0,1,0,-1};// Movimientos en el eje y (arriba, derecha, abajo e izquierda).

    for (int i = 0; i < 4; ++i) {// Recorrer los movimientos posibles y llamar recursivamente a la función.
        int newX=x+dx[i];// Calcular la nueva coordenada x de la casilla.
        int newY=y+dy[i];// Calcular la nueva coordenada y de la casilla.

        if (isValid(newX,newY) && !isInPath(newX,newY,path)){// Comprobar si la casilla es válida y si no está en el camino mínimo.
            int newCost=cost+abs(matriz[newX][newY]-matriz[x][y]);// Calcular el nuevo costo.
            if (newCost>=minCost){// Comprobar si el costo actual es mayor o igual que el costo mínimo encontrado hasta ahora.
                continue;
            }
            path.push_back({newX,newY});// Si el costo actual es menor que el costo mínimo encontrado hasta ahora, seguimos explorando.
            FindMinCostFour(newX,newY,newCost,path);// Llamada recursiva.
            path.pop_back();// Eliminar la casilla actual del camino mínimo.
        }
    }
}
